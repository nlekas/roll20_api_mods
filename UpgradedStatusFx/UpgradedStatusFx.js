...
var StatusFX = (function() {
     ...
     *           A mapping of token IDs to their UpdatedStatusFX state.
     */
    ...
    /**
     * The saved UpdatedStatusFX state for a token.
    ...
    function _initState() {
        ...
        state.UpdatedStatusFX = state.UpdatedStatusFX || {
            tokens: {},
            fx: {},
            dead:[],
        };
        ...
        state.UpdatedStatusFX.interval = useroptions.interval || 1000;
    ...
    function _initStatus(status, statuses) {
        ...
        if(match) {
            log('UpdatedStatusFX: ' + status + ' -> ' + match[0]);
            ...
            state.UpdatedStatusFX.fx[status] = {
                name: fxName,
                direction: direction
            };
        }
        else
            state.UpdatedStatusFX.fx[status] = undefined;
    }
    ...
    function getStatusFx(status) {
        return state.UpdatedStatusFX.fx[status];
    }

    /**
     * Gets the UpdatedStatusFX state for a particular token.
     * @param {graphic} token
     * @return {tokenStatusFX}
     */
    function getTokenStatusFx(token) {
        return state.UpdatedStatusFX.tokens[token.get('_id')];
    }
    ...
    function updateTokenState(token) {
        ...
        // On death animation to happen only once
        if(state.UpdatedStatusFX.dead.includes(token.get('_id')) && statuses.includes("dead")){
            delete statuses[statuses.findIndex(function(x){ return x === "dead"})];
        } else if(!state.UpdatedStatusFX.dead.includes(token.get('_id')) && statuses.includes("dead") ){
            state.UpdatedStatusFX.dead.push(token.get('_id'));
        } else if(state.UpdatedStatusFX.dead.includes(token.get('_id')) && !statuses.includes("dead")){
        ...
        setInterval(function() {
            ...
            _.each(tokens, function(token) {
                try {
                    ...
                    log('UpdatedStatusFX ERROR: ' + err.message);
                }
            });
        }, state.UpdatedStatusFX.interval);
    });
